...
PID_FILE="$STATE_DIR/mcp-voice-agent.pid"
LOG_FILE="$STATE_DIR/mcp-voice-agent.log"
SCRIPT_PATH="/home/t79/KITTY/bin/mcp-voice-agent.py"
...
if [[ -f "$PID_FILE" ]]; then
  ...
  if [[ -n "${PID:-}" ]] && kill -0 "$PID" 2>/dev/null && curl -fsS --max-time 2 "$HEALTH_URL" >/dev/null 2>&1; then
    echo "mcp-voice-agent already running"
    exit 0
  fi
  rm -f "$PID_FILE"
fi
