...
PID_FILE="$STATE_DIR/mcp-time-calendar.pid"
LOG_FILE="$STATE_DIR/mcp-time-calendar.log"
SCRIPT_PATH="/home/t79/KITTY/bin/mcp-time-calendar-agent.py"
...
if [[ -f "$PID_FILE" ]]; then
  ...
  if [[ -n "${PID:-}" ]] && kill -0 "$PID" 2>/dev/null && curl -fsS --max-time 2 "$HEALTH_URL" >/dev/null 2>&1; then
    echo "mcp-time-calendar already running"
    exit 0
  fi
  rm -f "$PID_FILE"
fi
...
echo "mcp-time-calendar started"
